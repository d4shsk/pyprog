...
2.  **Мемоизация** дает колоссальный прирост производительности при повторных вычислениях одних и тех же значений. Она меняет сложность повторного доступа с $O(N)$ на $O(1)$, однако требует дополнительной памяти для хранения кэша (Time-Space Trade-off).
